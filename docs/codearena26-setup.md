...
Run `npm ci --legacy-peer-deps` followed by `npm run dev`. When Cognito is absent, development enables explicit citizen, government, NGO and relief demo roles. `.data/hazards.json` stores cases, evidence, weather readings, area alerts, histories, reservations, opt-in locations, community invitations, reporting bans and feedback. Restarting the development server preserves them. This file and browser-test stores are ignored by Git. Version-1 stores initialize any missing optional collections without replacing cases or shelter reservations.
...
The Terraform configuration requires Terraform 1.6 or later, Node.js with the root dependencies installed, AWS credentials for the target account, and a `PowerShell` executable for the database bootstrap provisioner. Copy `infrastructure/terraform.tfvars.example` to the ignored `infrastructure/terraform.tfvars` and supply deployment values. Set `allowed_cidr_blocks` to the intended database clients; the sample's unrestricted range is not an access restriction. Email delivery requires a verified SES sender, and SMS requires the selected provider's configuration.

Build the Lambda dependencies and `dist/index.js` handlers before running Terraform plan or apply, including on a fresh checkout:
...
After reviewing the plan, `terraform -chdir=infrastructure apply` provisions the configured AWS resources. Terraform generates the Lambda ZIP archives in `infrastructure/build/` from the compiled source; those generated files are ignored by Git. Its database bootstrap runs both schema migrations and `db/seed.sql`, which loads demonstration records. Review the seed data before deploying a live operational database. The deployment host must be able to reach PostgreSQL for bootstrap.

## Live Node server

The hazard API is a Next.js Node route and cannot run in a static export. It uses Cognito authentication and a server-side PostgreSQL connection. Web and Android resource, SOS and news features also require the Terraform/AppSync deployment.

| Environment variable | Purpose |
|---|---|
| `NEXT_PUBLIC_AWS_REGION` | Cognito/AppSync region |
| `NEXT_PUBLIC_COGNITO_USER_POOL_ID` | Cognito pool; enables verified authentication |
| `NEXT_PUBLIC_COGNITO_USER_POOL_CLIENT_ID` | Expected ID-token audience |
| `NEXT_PUBLIC_APPSYNC_GRAPHQL_URL` | AppSync GraphQL endpoint |
...
Run the hazard-state migration against the database before starting the live server:

```sh
npm run db:hazards
```

This command reads `.env.local` if present and applies `db/migrations/002_hazards.sql` using `DATABASE_URL`. It does not seed, replace or delete existing application records. The Terraform bootstrap also includes this migration.
...
Those values are illustrative; replace them with confirmed operational data. Environment initialization never overwrites an existing state record. Hazard shelter reservations are a dedicated ledger; do not allocate the same unreserved capacity independently through the AppSync safe-zone dashboard. Reconcile that ledger before enabling both workflows for real dispatch.
...
Hazard warnings and closures are in-app broadcasts. The government SMS/email composer requires configured delivery providers. Weather replay does not send SMS or email. No cloud resources are provisioned by the web build or test commands.
...
Candidates come from OSRM, with alternatives requested. Complete segments and endpoint approaches are screened against recorded exclusion areas; confirmed hazards and current weather warnings block routes. AppSync disaster polygons must also be readable when that backend is configured. Incomplete geometry, unavailable providers, changed hazards, insufficient shelter capacity or an origin inside a risk buffer produce no screened route.
...
The web app uses Next.js 15, MapLibre 6 and Sharp 0.35, with a scoped PostCSS dependency override. MapLibre's worker and shared module are copied from the installed package by `predev` and `prebuild`. Map rendering needs WebGL 2; unsupported browsers retain readable case details and alerts.
