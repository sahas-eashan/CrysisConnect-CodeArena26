...
export const actorSchema = z.object({ id: z.string().trim().min(1).max(200), role: z.enum(["citizen", "ngo", "government"]), name: z.string().max(200).optional() });
...
export async function validatePhoto(value: unknown): Promise<{ dataUrl: string; mimeType: string; capturedAt?: string }> {
