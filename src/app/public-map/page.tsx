...
export default function PublicMapPage() {
  ...
  return <main className="mx-auto min-h-screen max-w-7xl space-y-6 px-4 py-8 sm:px-6">
    ...
    {snapshot ? <>
      <Card><CardTitle>{snapshot.hazards.length} verified active hazard{snapshot.hazards.length === 1 ? "" : "s"}</CardTitle><CardDescription className="mt-2">Red circles show precautionary exclusion areas around confirmed hazards. Green markers show shelters. Updated {new Date(snapshot.generatedAt).toLocaleString()}; refreshes every 10 seconds.</CardDescription>
